...
export default function SurveyAnalytics({ responses, questions, registrationCount = 0, registrationEnabled = false, isAr = false }: Props) {
        ...
        {registrationEnabled && (
          <>
            <div className="bg-feps-ink/5 p-4 border border-feps-ink/10 text-center">
              ...
              <p className="text-3xl font-serif text-feps-navy">{registrationCount}</p>
            </div>
            <div className="bg-feps-ink/5 p-4 border border-feps-ink/10 text-center">
              <p className="text-xs font-bold uppercase tracking-widest text-feps-ink-secondary mb-1">
                {isAr ? 'نسبة الاستجابة' : 'Response Completion Rate'}
              </p>
              <p className="text-3xl font-serif text-feps-navy">
                {registrationCount > 0 
                  ? `${Math.min(100, Math.round((responses.length / registrationCount) * 100))}%`
                  : '0%'}
              </p>
            </div>
          </>
